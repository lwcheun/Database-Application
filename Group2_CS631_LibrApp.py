...
class StartPage(tk.Frame):
  ...
  def widgets(self):
    ...
    panel.grid(row = 4, column = 3, rowspan = 4)
...
class ReaderMenu(tk.Frame):
  ...
  def update_print_reserved(self):  
    ...
    sql_input = "SELECT TITLE, STATUS, DTIME, DOCID, COPYNO, LIBID FROM DOCUMENT NATURAL JOIN RESERVES NATURAL JOIN COPY WHERE READERID ='%s'" % (self.reader_id)
    ...
    if self.res_arr != []:
      for i in range(len(self.res_arr)):
        if ("%s" % self.res_arr[i][1]) == "Reserved":
          ...
          dtime = self.res_arr[i][2]
          docid = self.res_arr[i][3]
          copyno = self.res_arr[i][4]
          libid = self.res_arr[i][5]
          self.show_res.insert(i, "Title:  %s" %title + ' '*20  
            + "Status:  %s" %status + ' '*20
            + "Reserve Date:  %s" %dtime + ' '*20
            + "DocID:  %s" %docid + ' '*20 
            + "CopyNo:  %s" %copyno + ' '*20  
            + "LibID:  %s" %libid + ' '*20 + '\n' )
  ...
  def check_cancel_time(self):
    ...
    for tup_time in check_valid:
      ...
      if tuple_date < dt.now().date(): #checked out before current day
        ...
        cancelled_arr.append( "Docid: %s" %copy_result[0][1] + ", CopyNo: %s" %copy_result[0][2] + ", Libid: %s" %copy_result[0][3] )
      elif tuple_date == dt.now().date():  #checked out in current day
        get_time = tup_time[0].split()[1][0:-1]
        tuple_time = dt.strptime(get_time, '%H:%M:%S').time()
        if tuple_time < time(18,0):  #checked out before 6pm of current day
          #nat join to get resnum's corr tuple in copy before deleting!!!
          sql_get_copy = "SELECT FREQUENCY, DOCID, COPYNO, LIBID FROM COPY NATURAL JOIN RESERVES WHERE RESNUMBER='%s'" % (tup_time[1])
          copy_result = self.controller.dbConnect(sql_get_copy, 'fetch')
          sql_input_2 = "UPDATE COPY SET STATUS='%s', FREQUENCY='%s' WHERE DOCID='%s' AND COPYNO='%s' AND LIBID='%s'" % ("Available", str(copy_result[0][0]), copy_result[0][1], copy_result[0][2], copy_result[0][3])
          ...
          cancelled_arr.append( "Docid: %s" %copy_result[0][1] + ", CopyNo: %s" %copy_result[0][2] + ", Libid: %s" %copy_result[0][3] )
    display_canc = ''
    for item in cancelled_arr:
      display_canc += item + '\n'
    self.popup_window("Cannot checkout/reserve; items cancelled due to time limit: " + display_canc)
    self.view_all()
    self.update_print_reserved()
    if cancelled_arr != []:
      return 'stop'
...
class AdminMenu(tk.Frame):
  ...
  def update_readers(self):  
    ...
    sql_input_2 = "SELECT READERID, RNAME, RTYPE, ADDRESS FROM READER NATURAL JOIN FINE WHERE FINE = 0"
